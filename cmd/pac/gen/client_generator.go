...
func (g *ClientGenerator) GenerateClient(inputReader io.Reader) ([]byte, error) {
	...
	// Add imports
	output.WriteString("import type { HabitatClient } from '../sdk/atproto';\n")
	output.WriteString("import type { CreateRecordResponse, GetRecordResponse, ListRecordsResponse } from '../sdk/atproto';\n")
	...
	g.generateCreateRecordFunction(&output, entityNamePascal, doc.ID)
	output.WriteString("\n")
	g.generateListRecordsFunction(&output, entityNamePlural, doc.ID)
	output.WriteString("\n")
	g.generateGetRecordFunction(&output, entityNamePascal, doc.ID)
	output.WriteString("\n\n")
	...
	g.generatePutPrivateRecordFunction(&output, entityNamePascal, doc.ID)
	output.WriteString("\n")
	g.generateGetPrivateRecordFunction(&output, entityNamePascal, doc.ID)
	output.WriteString("\n")
	g.generateListPrivateRecordsFunction(&output, entityNamePlural, doc.ID)

	return []byte(output.String()), nil
}

// generateCreateRecordFunction generates the createRecord function
func (g *ClientGenerator) generateCreateRecordFunction(output *strings.Builder, entityName string, lexiconID string) {
	output.WriteString(fmt.Sprintf("export const create%sRecord = async (client: HabitatClient, record: %s): Promise<CreateRecordResponse> => {\n", entityName, entityName))
...
func (g *ClientGenerator) generateListRecordsFunction(output *strings.Builder, entityNamePlural string, lexiconID string) {
	...
	output.WriteString(fmt.Sprintf("export const list%s = async (client: HabitatClient, repo?: string): Promise<ListRecordsResponse<%s>> => {\n", entityNamePlural, entityName))
...
func (g *ClientGenerator) generateGetRecordFunction(output *strings.Builder, entityName string, lexiconID string) {
	output.WriteString(fmt.Sprintf("export const get%sRecord = async (client: HabitatClient, rkey: string, repo?: string): Promise<GetRecordResponse<%s>> => {\n", entityName, entityName))
...
func (g *ClientGenerator) generatePutPrivateRecordFunction(output *strings.Builder, entityName string, lexiconID string) {
	output.WriteString(fmt.Sprintf("export const putPrivate%sRecord = async (client: HabitatClient, record: %s, rkey?: string): Promise<CreateRecordResponse> => {\n", entityName, entityName))
...
func (g *ClientGenerator) generateGetPrivateRecordFunction(output *strings.Builder, entityName string, lexiconID string) {
	output.WriteString(fmt.Sprintf("export const getPrivate%sRecord = async (client: HabitatClient, rkey: string, repo?: string): Promise<GetRecordResponse<%s>> => {\n", entityName, entityName))
...
func (g *ClientGenerator) generateListPrivateRecordsFunction(output *strings.Builder, entityNamePlural string, lexiconID string) {
	...
	entityName := g.singularize(entityNamePlural)
	output.WriteString(fmt.Sprintf("export const listPrivate%s = async (client: HabitatClient, repo?: string): Promise<ListRecordsResponse<%s>> => {\n", entityNamePlural, entityName))
