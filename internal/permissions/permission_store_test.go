...
func TestBasicPolicy1(t *testing.T) {
	...
	ok, err := ps.HasPermission("did:1", "did:owner", "app.bsky.likes", "like1")
	require.NoError(t, err)
	require.True(t, ok)
	ok, err = ps.HasPermission("did:1", "did:owner", "app.bsky.likes", "like2")
	require.NoError(t, err)
	require.True(t, ok)

	// glob match on nsid
	ok, err = ps.HasPermission("did:2", "did:owner", "app.bsky.videos", "video1")
	require.NoError(t, err)
	require.True(t, ok)
	ok, err = ps.HasPermission("did:2", "did:owner", "app.bsky.photos", "photo1")
	require.NoError(t, err)
	require.True(t, ok)
}

func TestListReadPermissionsByLexicon(t *testing.T) {
	a := fileadapter.NewAdapter("test_policies/test_policy_1.csv")
	ps, err := NewStore(a, false)
	require.NoError(t, err)

	ownerPerms, err := ps.ListReadPermissionsByLexicon("did:owner")
	require.NoError(t, err)
	require.Len(t, ownerPerms, 2)

	owner2Perms, err := ps.ListReadPermissionsByLexicon("did:owner2")
	require.NoError(t, err)
	require.Len(t, owner2Perms, 1)
}
