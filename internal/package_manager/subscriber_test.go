...
func TestAppInstallSubscriber(t *testing.T) {
	...
	lifeCycleSubscriber, err := NewAppLifecycleSubscriber(map[node.Driver]PackageManager{node.DriverNoop: pm}, nc)
	require.Equal(t, lifeCycleSubscriber.Name(), "AppLifecycleSubscriber")
	require.Nil(t, err)

	installAppExecutor := &InstallAppExecutor{
		packageManagers: map[node.Driver]PackageManager{node.DriverNoop: pm},
		nodeController:  nc,
	}
