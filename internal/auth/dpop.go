...
func NewDpopHttpClient(session *dpopSession, jwkBuilder DpopJWKBuilder) *DpopHttpClient {
...
func (s *DpopHttpClient) Do(req *http.Request) (*http.Response, error) {
	...
	if resp.Header.Get("DPoP-Nonce") != "" {
		s.session.setNonce(resp.Header.Get("DPoP-Nonce"))
	}
...
func (s *DpopHttpClient) Sign(req *http.Request) error {
	key, err := getDpopKey(s.session.session)
	if err != nil {
		return err
	}
