...
func setupTestRequestWithSession(t *testing.T, sessionStore sessions.Store, opts DpopSessionOptions) (*http.Request, *httptest.ResponseRecorder) {
	...
	dpopSession := &dpopSession{session: session, req: sessionReq, respWriter: sessionW}

	// Set all values directly without calling session.Save() multiple times
	dpopSession.setKey(keyBytes)
	dpopSession.setIdentity(identity)
	dpopSession.setPDSURL(opts.PdsURL)
...
func TestXrpcBrokerHandler_SuccessfulRequest(t *testing.T) {
	...
	req, w := setupTestRequestWithSession(t, sessionStore, DpopSessionOptions{
		...
		AccessToken:  stringPtr("test-access-token"),
...
func TestXrpcBrokerHandler_UnauthorizedResponse(t *testing.T) {
	...
	req, w := setupTestRequestWithSession(t, sessionStore, DpopSessionOptions{
		PdsURL:       mockPDS.URL,
		Issuer:       stringPtr("https://example.com"),
		AccessToken:  stringPtr("expired-access-token"),
		RefreshToken: stringPtr("test-refresh-token"),
	})
