...
func TestOAuthClient_RefreshToken_Success(t *testing.T) {
	...
	tokenResp, err := client.RefreshToken(dpopClient, dpopSession)
...
func TestOAuthClient_RefreshToken_GetIdentityError(t *testing.T) {
	server := fakeAuthServer(t, nil)
	defer server.Close()

	client := testOAuthClient(t)
	// Create session without identity data
	dpopSession := testDpopSession(t, DpopSessionOptions{
		PdsURL:         server.URL,
		Issuer:         stringPtr("https://example.com"),
		RefreshToken:   stringPtr("old-refresh-token"),
	...
	_, err := client.RefreshToken(dpopClient, dpopSession)

	require.Error(t, err)
	require.Contains(t, err.Error(), "session value not found")
}
...
func TestOAuthClient_RefreshToken_GetIssuerError(t *testing.T) {
	server := fakeAuthServer(t, nil)
	defer server.Close()

	client := testOAuthClient(t)
	// Create session without issuer
	dpopSession := testDpopSession(t, DpopSessionOptions{
		PdsURL:       server.URL,
		Issuer:       nil, // Omit issuer
		RefreshToken: stringPtr("old-refresh-token"),
	})
	dpopClient := testDpopClientFromSession(t, dpopSession)
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to get issuer")
}

func TestOAuthClient_RefreshToken_GetRefreshTokenError(t *testing.T) {
	server := fakeAuthServer(t, nil)
	defer server.Close()
	...
	dpopSession := testDpopSession(t, DpopSessionOptions{
		PdsURL:       server.URL,
		Issuer:       stringPtr("https://example.com"),
		RefreshToken: nil, // Omit refresh token
	})
	dpopClient := testDpopClientFromSession(t, dpopSession)
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to get refresh token")
}
...
func TestOAuthClient_RefreshToken_InvalidJSONResponse(t *testing.T) {
	server := fakeAuthServer(t, map[string]interface{}{
		"token": "invalid json",
	})
	defer server.Close()
...
func TestOAuthClient_RefreshToken_EmptyResponse(t *testing.T) {
	server := fakeAuthServer(t, map[string]interface{}{
		"token": "",
	})
	defer server.Close()
