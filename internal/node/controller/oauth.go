...
func NewHabitatOAuthProxy(domain string, authPersister oauth.Persister) *HabitatOAuthProxy {
	...
	pdsURL := "http://host.docker.internal:5001"

	oauthConfig := oauth.Config{
		Protocol: "https",
		Host:     domain,
		// TODO @eagraf we need a flow for generating this
...
func (h *HabitatOAuthProxy) xrpcBrokerHandler(w http.ResponseWriter, r *http.Request) {
	externalURL := fmt.Sprintf("https://%s", r.Host)
	broker := oauth.NewTokenBroker(h.persister, h.oauthConfig.Host, h.oauthConfig.PDSURL, externalURL)
